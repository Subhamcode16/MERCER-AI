      ...
      <div className="flex items-center justify-between border-b border-border pb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 rounded text-[10px] font-mono uppercase bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-500/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {attributions.map((item) => (
          <div
            ...
            className="bg-card border border-border rounded-xl p-6 space-y-4 shadow-xs hover:border-cyan-500/30 transition-all"
          >
            <div className="flex items-center justify-between">
              <span className="font-mono text-xs font-semibold text-foreground">{item.campaign_id}</span>
            ...
            <div className="grid grid-cols-3 gap-3 p-3 bg-muted/40 rounded-lg border border-border font-mono text-center">
              <div>
                <div className="text-muted-foreground text-[10px] uppercase">Raw CTR Lift</div>
                <div className="text-lg font-bold text-foreground mt-0.5">+{(item.raw_lift * 100).toFixed(1)}%</div>
              </div>
              <div>
                <div className="text-muted-foreground text-[10px] uppercase">Confounder Score</div>
                <div className="text-lg font-bold text-amber-600 dark:text-amber-400 mt-0.5">{(item.confounder_score * 100).toFixed(0)}%</div>
              </div>
              <div>
                <div className="text-muted-foreground text-[10px] uppercase">Isolated Lift</div>
                <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">+{(item.calibrated_lift * 100).toFixed(1)}%</div>
              </div>
            </div>

            <div className="space-y-1.5 text-xs font-mono">
              <div className="text-muted-foreground text-[11px]">Confounders Isolated:</div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 space-y-6 shadow-xs">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {calibrations.map((cal) => (
            <div
              key={cal.model_id}
              className="p-5 bg-muted/30 rounded-xl border border-border space-y-4 font-mono text-xs"
            >
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-semibold text-foreground text-sm">{cal.model_id}</div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">Brier Score (Lower is better):</span>
                  <span className="text-foreground font-bold">{cal.brier_score}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">Overconfidence Penalty:</span>
                  <span className="text-amber-600 dark:text-amber-400">-{cal.overconfidence_penalty}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">Empirical Accuracy vs Confidence:</span>
                  <span className="text-emerald-600 dark:text-emerald-400">{(cal.empirical_accuracy * 100).toFixed(0)}% vs {(cal.predicted_confidence_mean * 100).toFixed(0)}%</span>
                </div>
              </div>
            </div>
          ))}
