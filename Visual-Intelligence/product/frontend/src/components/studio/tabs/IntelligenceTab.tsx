  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div className="p-6 rounded-2xl bg-card border border-border space-y-4 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-serif text-foreground font-medium">Pre-Generation Creative Intelligence</h2>
            </div>
        ...
        <div className="p-3.5 rounded-xl bg-muted/40 border border-border flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500 dark:bg-emerald-400 animate-pulse" />
            <span>Invariant Guard: <strong className="text-foreground">Model Output &ne; Truth</strong> &bull; <strong className="text-foreground">Unknown Must Survive</strong></span>
          </div>
          <span className="text-[10px] font-mono text-muted-foreground/60">Zero Certainty Fabrication Policy</span>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-muted-foreground/70 uppercase tracking-wider">{item.category}</span>
                ...
                <p className="text-xs text-muted-foreground line-clamp-2 font-light">{item.summary}</p>
              </div>
            );
          })}
        </div>
          ...
          {activeItem ? (
            <div className="p-6 rounded-2xl bg-card border border-border space-y-6 sticky top-24 shadow-sm">
              ...
              <div className="flex items-start justify-between gap-4 border-b border-border pb-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono uppercase text-primary font-bold">{activeItem.category}</span>
                  ...
                  <h3 className="text-base font-serif text-foreground font-medium">{activeItem.title}</h3>
                </div>
              </div>

              {/* Summary Description */}
              <div className="space-y-1.5">
                <span className="text-[10px] font-mono uppercase text-muted-foreground/70 tracking-wider">Empirical Finding / Signal</span>
                <p className="text-xs text-foreground/90 leading-relaxed font-light">{activeItem.summary}</p>
              </div>
              ...
              <div className="p-4 rounded-xl bg-muted/40 border border-border space-y-2">
                <span className="text-[10px] font-mono uppercase text-muted-foreground/70 tracking-wider flex items-center gap-1.5">
                  <Database className="w-3 h-3 text-primary" /> Provenance & Data Origin
                </span>
                <p className="text-xs text-primary font-mono">{activeItem.evidenceSource}</p>
              </div>
              ...
              {activeItem.tensionNote && (
                <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 space-y-1 text-xs">
                  <div className="flex items-center gap-1.5 text-amber-700 dark:text-amber-300 font-semibold text-[11px]">
                    <ShieldAlert className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" /> Strategic Tension / Physical Constraint
                  </div>
                  <p className="text-amber-800 dark:text-amber-200 font-light">{activeItem.tensionNote}</p>
                </div>
              )}
              ...
              <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 space-y-1 text-xs">
                <span className="text-[10px] font-mono uppercase text-emerald-600 dark:text-emerald-400 tracking-wider font-semibold">
                  &rarr; Downstream Synthesis Constraint
                </span>
                <p className="text-foreground/90 font-light">{activeItem.actionableImplication}</p>
              </div>
