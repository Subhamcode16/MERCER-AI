...
export const AgentRosterPanel: React.FC<AgentRosterPanelProps> = ({
  ...
  return (
    <div className="relative h-full flex shrink-0 z-30">
      {/* Collapsed Icon Bar */}
      <div className="w-12 h-full border-l border-border bg-card/90 flex flex-col items-center py-6 gap-6 shrink-0 z-40">
        <button
            ...
            return (
              <button
                ...
                className={`relative hover:scale-110 transition-transform cursor-pointer rounded-full ${
                  isProcessing ? "ring-2 ring-primary ring-offset-2 ring-offset-background" : ""
                }`}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="h-full border-l border-border bg-card/95 flex flex-col shrink-0 overflow-y-auto px-6 py-8 gap-6 backdrop-blur-xl shadow-2xl relative text-left"
          >
            <div className="flex justify-between items-center pb-2 border-b border-border">
              <span className="text-[9px] font-mono tracking-widest text-primary uppercase font-bold flex items-center gap-2">
                <Users size={12} />
                Governed Workforce Roster
              </span>
              <button
                onClick={() => setIsOpen(false)}
                className="w-6 h-6 rounded-full border border-border flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors cursor-pointer"
              >
            ...
            <div className="flex flex-col gap-3">
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-[8px] font-mono text-emerald-600 dark:text-emerald-400/80 uppercase flex items-center gap-1 font-medium">
                  <ShieldCheck size={10} /> Active
                </span>
              </div>

              {brandDna ? (
                <div className="relative rounded-2xl border border-primary/20 bg-muted/20 p-4 overflow-hidden shadow-sm">
                  <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-primary/30 to-transparent" />
                  <h3 className="font-serif text-[12px] text-foreground font-medium leading-snug tracking-wide mb-2">
                    {brandDna.account}
                  </h3>
                  ...
                  <div className="space-y-2 font-mono text-[8.5px]">
                    <div className="flex flex-col">
                      ...
                      <span className="text-foreground/90 font-light mt-0.5">
                        {brandDna.archetype}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex gap-1.5 mt-1">
                          {brandDna.palette.map((color, i) => (
                            <div
                              key={i}
                              className="flex items-center gap-1 bg-background border border-border px-1.5 py-0.5 rounded shadow-xs"
                            >
                              <span
                                className="w-1.5 h-1.5 rounded-full shrink-0"
                                style={{ backgroundColor: color }}
                              />
                              <span className="text-[7.5px] uppercase tracking-wide text-muted-foreground">
                                {color}
                              </span>
                            </div>
                          ))}
            ...
            <div className="flex flex-col gap-3">
              <span className="text-[8px] font-mono tracking-widest text-muted-foreground uppercase flex items-center gap-1.5">
                <Layers size={10} />
              ...
              <div className="flex flex-col gap-3">
                {depts.map((dept) => {
                  ...
                  return (
                    <div
                      key={dept}
                      className="border border-border rounded-xl overflow-hidden bg-muted/10 shadow-xs"
                    >
                      {/* Department Accordion Header */}
                      <button
                        onClick={() => toggleDept(dept)}
                        className="w-full px-3 py-2.5 flex items-center justify-between bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer border-b border-border"
                      >
                        <span className="text-[9px] font-mono font-semibold tracking-wider text-primary uppercase flex items-center gap-1.5">
                      ...
                      {isDeptOpen && (
                        <div className="p-2 flex flex-col gap-2 bg-muted/20">
                          {deptStaff.map((agent) => {
                            ...
                            const authorityBadgeColor =
                              agent.authority === "PROPOSE"
                                ...
                                : "border-indigo-500/30 text-indigo-700 dark:text-indigo-300 bg-indigo-500/10";

                            return (
                              <button
                                key={agent.id}
                                onClick={() => openDossierForAgent(agent)}
                                className={`w-full text-left flex flex-col gap-1.5 bg-card hover:bg-muted/60 border p-2.5 rounded-lg transition-all cursor-pointer group shadow-xs ${
                                  isProcessing
                                  ...
                                  <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <div className="flex flex-col min-w-0">
                                      <div className="flex items-center gap-1">
                                        <span className="text-[9px] font-mono text-foreground group-hover:text-primary font-medium truncate transition-colors">
                                          {agent.handle}
                                        </span>
                                        <FileText size={9} className="text-muted-foreground group-hover:text-primary/70 shrink-0" />
                                      </div>
                                      <span className="text-[7.5px] font-sans text-muted-foreground truncate">
                                        {agent.role}
                                      </span>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span
                                      className={`text-[6.5px] font-mono tracking-wider uppercase px-1 py-0.5 rounded border ${
                                        isProcessing
                                          ? "bg-purple-500/10 border-purple-500/30 text-purple-700 dark:text-purple-300 animate-pulse"
                                          : isCompleted
                                ...
                                {task.currentTask && (
                                  <div className="pl-7 mt-0.5">
                                    {isProcessing ? (
                                      ...
                                      <p className="text-[7.5px] font-mono text-emerald-600 dark:text-emerald-400/80 truncate">
                                        ✓ {task.currentTask}
                                      </p>
                                    )}
                                  </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              {generatedAssets.length === 0 ? (
                <div className="border border-dashed border-border rounded-2xl p-6 text-center">
                  <ImageIcon className="w-5 h-5 text-muted-foreground/30 mx-auto mb-2" />
                  <p className="text-[8.5px] font-mono tracking-wider text-muted-foreground uppercase">
                    Vault Empty
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-2">
                  {generatedAssets.map((asset, i) => (
                    <div
                      key={i}
                      className="aspect-square bg-muted border border-border rounded-xl overflow-hidden relative group cursor-pointer hover:border-primary/40 transition-all duration-300 shadow-xs"
                    >
                      <img
                        src={asset.url}
                        alt="output"
