  ...
  return (
    <li className="flex items-start space-x-3 py-2">
      ...
      <span className={cn("text-sm", feature.isIncluded ? "text-foreground" : "text-muted-foreground")}>
        {feature.stringValue ? (
          <><span className="font-semibold text-foreground">{feature.stringValue}</span> {feature.name}</>
        ) : (
          feature.name
        )}
      </span>
...
export const PricingComponent: React.FC<PricingComponentProps> = ({
  ...
  const CycleToggle = (
    <div className="flex justify-center mb-10 mt-2">
      <ToggleGroup
        ...
        className="rounded-lg p-1 bg-muted/30 dark:bg-muted/20"
      >
        <ToggleGroupItem
          value="monthly"
      ...
      {plans.map((plan) => {
        ...
        return (
          <Card
            key={plan.id}
            className={cn(
              "flex flex-col transition-all duration-300 shadow-md hover:shadow-[0_0_40px_rgba(225,212,192,0.15)] bg-background/40 backdrop-blur-xl border-border/50",
              isFeatured && "ring-2 ring-primary dark:ring-primary/80 shadow-[0_0_20px_rgba(225,212,192,0.1)] transform md:scale-[1.02] hover:scale-[1.04] hover:shadow-[0_0_50px_rgba(225,212,192,0.25)] bg-background/60"
            )}
          >
  ...
  const ComparisonTable = (
    <div className="mt-16 hidden md:block border rounded-lg overflow-x-auto shadow-sm dark:border-border/50 bg-background/40 backdrop-blur-xl">
          ...
          {allFeatures.map((featureName, index) => (
            <tr key={featureName} className={cn("transition-colors hover:bg-accent/20 dark:hover:bg-accent/10", index % 2 === 0 ? "bg-background" : "bg-muted/10 dark:bg-muted/5")}>
              ...
              {plans.map((plan) => {
                ...
                const stringValue = feature?.stringValue;
                ...
                return (
                  ...
                  >
                    {stringValue ? (
                      <span className="text-sm font-medium text-foreground">{stringValue}</span>
                    ) : (
  ...
  // --- 3.4. Final Render ---
  return (
    <div className={cn("w-full pb-12 md:pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8", className)} {...props}>
      {CycleToggle}
